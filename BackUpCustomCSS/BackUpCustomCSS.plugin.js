...
const {showToast, React: {createElement}, showConfirmationModal, alert, Plugins: {get, folder}} = BdApi
const {error} = console
...
module.exports = class BackUpCustomCSS{
    ...
    observer() {
        if (document.getElementById("bd-editor-controls") && !document.getElementById("BackUpCustomCSS")) {
            ...
            ele.onclick = (e) => {
                if (e.shiftKey === true) {
                    showConfirmationModal('Back up custom CSS settings', get('BackUpCustomCSS').instance.getSettingsPanel(), {
                        confirmText: "Close",
                        cancelText: null
                    })
                } else this.backup()
